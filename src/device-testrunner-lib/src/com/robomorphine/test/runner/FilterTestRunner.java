...
public class FilterTestRunner extends InstrumentationTestRunner {
    ...
    private FilterPredicate mFilter = null;

    @Override
    public void onCreate(Bundle args) {
        if (args.containsKey(ARGUMENT_TEST_SIZE_PREDICATE)) {
            String msg = String.format(
                    "Please do not use \"%s\" argument, it's not valid with this runner. "
                            + "Use \"%s\" argument instead.", ARGUMENT_TEST_SIZE_PREDICATE,
                    ARGUMENT_TEST_FILTER_PREDICATE);

            throw new IllegalArgumentException(msg);
        }
        mFilter = new FilterPredicate(args.getString(ARGUMENT_TEST_FILTER_PREDICATE));
        super.onCreate(args);
    };

    protected List<Predicate<TestMethod>> getBuilderRequirements() {
